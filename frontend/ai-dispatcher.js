  ...
  function _showLoadsInChat(loads){
    ...
    var inner = loads.map(function(l){
      ...
      return '<div style="background:#fff;border:1.5px solid #e0e0e0;border-radius:10px;padding:12px;margin-bottom:8px;">' +
        ...
        '<button onclick="(function(k){closeAI();setTimeout(function(){if(typeof openCargo!==\'function\')return;var d=window._aiDispatcherLoads&&window._aiDispatcherLoads[k];var c=(d&&d.id&&window.allLoads&&window.allLoads.find(function(x){return x.id==d.id;}))||d;if(c)openCargo(c);},200);})(\''+loadKey+'\')" style="margin-top:8px;width:100%;background:#f7b731;color:#1a1a2e;border:none;padding:7px;border-radius:7px;font-size:13px;font-weight:700;cursor:pointer;">Посмотреть и откликнуться →</button>' +
        '</div>';
    }).join('');
    div.innerHTML = inner;
    msgs.appendChild(div);
