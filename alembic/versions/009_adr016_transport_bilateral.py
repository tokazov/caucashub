...
def upgrade():
    """Идемпотентная миграция — таблицы могут быть уже созданы emergency_migrations."""
    from sqlalchemy import inspect as sa_inspect
    bind = op.get_bind()
    tables = sa_inspect(bind).get_table_names()

    if 'transport_offers' not in tables:
        op.create_table(
            'transport_offers',
    ...
    if 'transport_requests' not in tables:
        op.create_table(
            'transport_requests',
            sa.Column('id',                  sa.Integer, primary_key=True),
    ...
    if 'transport_subscriptions' not in tables:
        op.create_table(
    ...
    # deals: добавляем колонки если их нет
    try:
        op.add_column('deals',
            sa.Column('transport_offer_id', sa.Integer,
...
def downgrade():
    ...
    try:
        ...
        pass
    for idx in ['ix_transport_sub_route', 'ix_transport_sub_active', 'ix_transport_sub_user']:
        try:
            op.drop_index(idx, table_name='transport_subscriptions')
        except Exception:
            pass
    try:
        ...
        pass
    for idx in ['ix_transport_requests_status', 'ix_transport_requests_user_id', 'ix_transport_requests_offer_id']:
        try:
            op.drop_index(idx, table_name='transport_requests')
        except Exception:
            pass
    try:
        ...
        pass
    for idx in ['ix_transport_offers_status', 'ix_transport_offers_user_id']:
        try:
            op.drop_index(idx, table_name='transport_offers')
        except Exception:
            pass
    try:
